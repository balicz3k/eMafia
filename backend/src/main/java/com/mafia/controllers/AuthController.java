...
import com.mafia.dto.auth.RefreshTokenReq;
...
public class AuthController {
  ...
  @Operation(
      ...
      description =
          "Creates a new user account with username, email and password. Email must be unique in the system.",
        ...
        @ApiResponse(
            ...
            content =
                @Content(
                    ...
                    schema = @Schema(implementation = AuthResp.class),
                    examples =
                        @ExampleObject(
                            ...
                            value =
                                """
                    {
                        "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpJVCL9...",
                        "type": "Bearer",
                        "user": {
  ...
  public ResponseEntity<AuthResp> register(
      @Parameter(
              ...
              content =
                  @Content(
                      mediaType = "application/json",
                      schema = @Schema(implementation = RegistrationReq.class),
                      examples =
                          @ExampleObject(
  ...
  @Operation(
      summary = "User login",
      description =
          "Authenticates user with email and password, returns JWT token for subsequent API calls",
  ...
  public ResponseEntity<AuthResp> refreshToken(
      @Parameter(description = "Refresh token request", required = true) @RequestBody
          RefreshTokenReq request) {

    try {
      AuthResp response = refreshTokenService.refreshAccessToken(request.getRefreshToken());
      return ResponseEntity.ok(response);
  ...
  @Operation(
      summary = "Logout user",
      description =
          "Revoke refresh token and logout current session. Requires providing the refresh token in the request body.")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "204", description = "Successfully logged out"),
        @ApiResponse(
            responseCode = "400",
            description = "Bad Request - Refresh token might be missing if required by logic",
            content = @Content),
        @ApiResponse(
            responseCode = "401",
            description = "Unauthorized - Valid access token required in Authorization header",
            content = @Content)
      })
  public ResponseEntity<Void> logout(
      @Parameter(description = "Logout request with refresh token") @Valid @RequestBody
          LogoutReq request) {
    if (request == null
